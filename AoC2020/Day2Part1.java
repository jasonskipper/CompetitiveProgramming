...
public class Day2Part1 {
	public static void main(String[] args) {
		String input = "15-19 k: kkkkkkkkkkkkzkkkkkkk\n" + 
		...
		for(int i = 0; i < items.length; i++) {
			String[] splititems = items[i].split(" ");
			String[] numbers = splititems[0].split("-");
			int lower = Integer.parseInt(numbers[0]);
			int higher = Integer.parseInt(numbers[1]);
			String middle = splititems[1].replaceAll(":", "");
			char more_middle = middle.charAt(0);
			String searchspace = splititems[2];
			int count = 0;
			for(int j = 0; j < searchspace.length(); j++) {
				if(searchspace.charAt(j) == more_middle) {
					count += 1;
				}
			}
			// I almost changed my code entirely because I used || instead of && at first 
